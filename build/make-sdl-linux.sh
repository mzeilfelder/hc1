...
cd ${LIB_TARGET}/build
$LIB_SOURCE/configure CPPFLAGS=-O2 --prefix=$LIB_TARGET --enable-static=no --enable-audio=no --enable-video=no --enable-cdrom=no --enable-threads=no --enable-file=no --enable-loadso=no 
make
make install
